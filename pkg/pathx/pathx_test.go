...
func TestFromExecutable(t *testing.T) {
	...
	for i, v := range tests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			if v.err != nil {
				assert.PanicsWithError(t, v.err.Error(), func() { pathx.FromExecutable(v.relPath, executable) })
			} else {
				abspath := pathx.FromExecutable(v.relPath, executable)
...
func TestInsertSuffix(t *testing.T) {
	...
	for _, v := range tests {
		t.Run(v.name, func(t *testing.T) {
			res := pathx.InsertFsuffix(v.path, v.suffix)
