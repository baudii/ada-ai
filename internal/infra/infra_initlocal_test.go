...
func TestInitLocalProject_FailsProjectFolder(t *testing.T) {
	t.Parallel()
	lp, err := infra.NewFSProject(t.TempDir(), project.Data{}, 0, mdpErr)
	assert.Nil(t, lp)
...
func TestInitLocalProject_FailsToCreateProjectManager(t *testing.T) {
	...
	_ = f.Close()
	ai, err := infra.NewFSProject(t.TempDir(), project.Data{}, seqdir.CreateNew, &mockDirProvider{path: d})
	assert.Nil(t, ai)
...
func TestInitLocalProject_Success(t *testing.T) {
	t.Parallel()
	ai, err := infra.NewFSProject(t.TempDir(), project.Data{}, seqdir.CreateNew, &mockDirProvider{path: t.TempDir()})
	require.NotNil(t, ai)
	require.NoError(t, err)
}
